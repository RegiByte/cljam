...
describe('env', () => {
  ...
  it('should define a new binding', () => {
    const env = makeEnv()
    define('x', v.number(1), env)
    expect(env.bindings.get('x')).toMatchObject(v.number(1))
  })

  it('should lookup a binding in the environment', () => {
    const env = makeEnv()
    define('x', v.number(1), env)
    expect(lookup('x', env)).toMatchObject(v.number(1))
  })

  it('should extend an environment with new bindings', () => {
    const env = makeEnv()
    define('x', v.number(1), env)
    const extended = extend(['y'], [v.number(2)], env)
    ...
    expect(lookup('x', extended)).toMatchObject(v.number(1))
    expect(lookup('y', extended)).toMatchObject(v.number(2))
    expect(extended.outer!.bindings.get('x')).toMatchObject(v.number(1)) // x is in the parent environment
    expect(extended.bindings.get('y')).toMatchObject(v.number(2)) // y is in the extended environment
  ...
  it('should shadow parent bindings in the child env', () => {
    const outer = makeEnv()
    define('x', v.number(1), outer)
    const inner = extend(['x'], [v.number(99)], outer)
    expect(lookup('x', inner)).toMatchObject(v.number(99))
    expect(lookup('x', outer)).toMatchObject(v.number(1))
  })
})
