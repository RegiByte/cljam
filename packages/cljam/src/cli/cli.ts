...
import { inferSourceRoot, discoverSourceRoots, discoverMain, parseMainSpec, discoverSessionProfile } from '../nrepl/nrepl-utils'
import { startNreplServer } from '../nrepl/nrepl'
...
export async function runCli(
  ...
  if (command === 'nrepl-server') {
    ...
    const profile = discoverSessionProfile(effectiveRoot)

    const libraries = await loadLibrariesFromRoot(effectiveRoot)
    const mainStr = discoverMain(effectiveRoot)

    const importModule = (s: string) => import(s)

    if (libraries.length > 0 || mainStr) {
      // Pre-build a warm session so libraries and the main namespace are
      // already loaded when the server snapshots it for clones.
      const session = createSession({
        sourceRoots,
        readFile: (filePath) => readFileSync(filePath, 'utf8'),
        libraries,
      ...
      if (mainStr) {
        try {
          const { ns, fn } = parseMainSpec(mainStr)
          await session.evaluateAsync(`(require '[${ns}])`)
          session.setNs(ns)
          if (fn) {
            await session.evaluateAsync(`(${ns}/${fn})`)
          }
          process.stderr.write(`[cljam] loaded main: ${mainStr}\n`)
        } catch (e) {
          process.stderr.write(
            `[cljam] warning: failed to load main '${mainStr}': ${e instanceof Error ? e.message : e}\n`
          )
        }
      }

      startNreplServer({ port, host, session, profile, importModule, onOutput: (t) => process.stdout.write(t) })
    } else {
      startNreplServer({ port, host, sourceRoots, profile, importModule })
    }
